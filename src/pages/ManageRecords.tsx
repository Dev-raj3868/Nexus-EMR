...
const ManageRecords = () => {
  ...
  const [searchDate, setSearchDate] = useState(today); // ✅ default today
  ...
  const handleSearch = () => {
    ...
    const filtered = prescriptions.filter((p) => {
      const matchId =
        !searchId || p.id.toLowerCase().includes(searchId.toLowerCase());
      ...
      const matchPhone =
        !searchPhone || p.patients?.phone.includes(searchPhone);

      const matchDate =
        !searchDate ||
        format(new Date(p.created_at), "yyyy-MM-dd") === searchDate;
  ...
  const handleClear = () => {
    ...
    setSearchName("");
    setSearchPhone("");
    setSearchDate(today); // ✅ reset to today
        ...
        <CardContent>
          ...
          {!hasSearched && (
            <p className="text-center py-10 text-muted-foreground">
              Please search to view prescription records
            </p>
          )}
          ...
          {selectedPrescription && (
            <div className="space-y-4">
              <p>
                <b>Date:</b>{" "}
                {format(
                  new Date(selectedPrescription.created_at),
                  "dd/MM/yyyy HH:mm"
                )}
              </p>
